...
class classy_meta_box {
	...
	function js_css() {
		...
		wp_enqueue_style('classy-meta-box', $base_url . 'meta-box.css');
		wp_enqueue_script('classy-meta-box', $base_url . 'meta-box.js', array('jquery'), null, true);
	}
	...
	function insert_images() {
		if (!isset($_POST['classy-insert']) || empty($_POST['attachments'])) return;
	...
	function check_field_date() {
		if ($this->has_field('date') && $this->is_edit_page()) {
			...
			wp_enqueue_style('classy-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('classy-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
		}
	}
	...
	function check_field_time() {
		if ($this->has_field('time') && $this->is_edit_page()) {
			...
			wp_enqueue_style('classy-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('classy-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
			wp_enqueue_script('classy-timepicker', 'https://github.com/trentrichardson/jQuery-Timepicker-Addon/raw/master/jquery-ui-timepicker-addon.js', array('classy-jquery-ui'));
		}
	}
	...
	function show() {
		...
		wp_nonce_field(basename(__FILE__), 'classy_meta_box_nonce');
	...
	function show_field_begin($field, $meta) {
		echo "<th class='classy-label-td'><label class='classy-label' for='{$field['id']}'>{$field['name']}</label></th><td class='classy-field' style='position: relative;'>";
	}
	...
	function show_field_text($field, $meta) {
		...
		echo "<input type='text' class='classy-text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_textarea($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<textarea class='classy-textarea large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='5'>$meta</textarea>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_select($field, $meta) {
		...
		$this->show_field_begin($field, $meta);
		echo "<select class='classy-select' name='{$field['id']}" . ($field['multiple'] ? "[]' id='{$field['id']}' multiple='multiple'" : "'") . ">";
	...
	function show_field_radio($field, $meta) {
		$this->show_field_begin($field, $meta);
		foreach ($field['options'] as $key => $value) {
			echo "<input type='radio' class='classy-radio' name='{$field['id']}' value='$key'" . checked($meta, $key, false) . " /> $value ";
		}
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_checkbox($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='checkbox' class='classy-checkbox' name='{$field['id']}' id='{$field['id']}'" . checked(!empty($meta), true, false) . " /> {$field['desc']}</td>";
	}

	function show_field_wysiwyg($field, $meta) {
		$this->show_field_begin($field, $meta);
		//wp_tiny_mce(true, array('editor_selector' => 'classy-rich-editor', 'remove_linebreaks' => false) );
		echo "<textarea class='classy-wysiwyg classy-rich-editor large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='10'>$meta</textarea>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_file($field, $meta) {
		...
		if (!empty($meta)) {
			$nonce = wp_create_nonce('rw_ajax_delete');
			echo '<div style="margin-bottom: 10px"><strong>' . __('Uploaded files', 'classy') . '</strong></div>';
			echo '<ol class="classy-upload">';
			foreach ($meta as $att) {
				// if (wp_attachment_is_image($att)) continue; // what's image uploader for?
				echo "<li>" . wp_get_attachment_link($att, '' , false, false, ' ') . " (<a class='classy-delete-file' href='#' rel='$nonce|{$post->ID}|{$field['id']}|$att'>" . __('Delete', 'classy') . "</a>)</li>";
			}
			echo '</ol>';
		}

		// show form upload
		echo "<div style='clear: both'><strong>" . __('Upload new files', 'classy') . "</strong></div>
			<div class='new-files'>
				<div class='file-input'><input type='file' name='{$field['id']}[]' /></div>
				<a class='classy-add-file' href='#'>" . __('Add more file', 'classy') . "</a>
			</div>
		</td>";
	}
	...
	function show_field_image($field, $meta) {
		...
		echo "<input type='hidden' class='classy-images-data' value='{$post->ID}|{$field['id']}|$nonce_sort' />
			  <ul class='classy-images classy-upload' id='classy-images-{$field['id']}'>";
		...
		foreach ($images as $image) {
			...
			echo "<li id='item_$image'>
					<img src='$src' />
					<a title='" . __('Delete this image', 'classy') . "' class='classy-delete-file' href='#' rel='$nonce_delete|{$post->ID}|{$field['id']}|$image'>" . __('Delete', 'classy') . "</a>
					<input type='hidden' name='{$field['id']}[]' value='$image' />
				</li>";
		}
		echo '</ul>';

		echo "<a href='#' class='classy-upload-button button' rel='{$post->ID}|{$field['id']}'>" . __('Add more images', 'classy') . "</a>";
	}

	function show_field_color($field, $meta) {
		if (empty($meta)) $meta = '#';
		$this->show_field_begin($field, $meta);
		echo "<input class='classy-color' type='text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='8' />
			  <a href='#' class='classy-color-select' rel='{$field['id']}'>" . __('Select a color', 'classy') . "</a>
			  <a href='#' class='classy-color-select' style='display: none;'>" . __('Hide Picker', 'classy') . "</a>
			  <div style='display:none; position: absolute; background: #f0f0f0; border: 1px solid #ccc; z-index: 100;' class='classy-color-picker' rel='{$field['id']}'></div>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_checkbox_list($field, $meta) {
		...
		$html = array();
		foreach ($field['options'] as $key => $value) {
			$html[] = "<input type='checkbox' class='classy-checkbox_list' name='{$field['id']}[]' value='$key'" . checked(in_array($key, $meta), true, false) . " /> $value";
		}
	...
	function show_field_date($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='classy-date' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_time($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='classy-time' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_single_image($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='classy-single-image' name='{$field['id']}' id='{$field['id']}' value='$meta' size='18' />";
		echo "<input type='button' class='classy-upload-single-image button-secondary' value='Choose Image'/><br />";
		$this->show_field_end($field, $meta);
	}
	
	function show_field_slider($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<div class='classy-slider' rel='{$field['id']}'></div>";		
	...
	function save($post_id) {
		...
		|| (!check_admin_referer(basename(__FILE__), 'classy_meta_box_nonce'))		// verify nonce
		...
		foreach ($this->_fields as $field) {
			...
			// validate meta value
			if (class_exists('classy_meta_box_Validate') && method_exists('classy_meta_box_Validate', $field['validate_func'])) {
				$new = call_user_func(array('classy_meta_box_Validate', $field['validate_func']), $new);
			}
